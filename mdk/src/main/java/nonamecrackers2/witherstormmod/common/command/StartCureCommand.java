      ...
      LiteralArgumentBuilder<CommandSourceStack> setPhaseCommand = (LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)Commands.literal(
         ...
         .then(
            Commands.literal("sickness")
               .then(Commands.literal("cure").then(Commands.argument("entity", EntityArgument.entity()).executes(StartCureCommand::startCure)))
         );
               ...
               if (optional.isPresent()) {
                  ...
                  if (!tracker.isBeingCured()) {
                     if (tracker.isInfected()) {
                        MutableComponent component = Component.translatable("commands.witherstormmod.sickness.startCure", new Object[]{living.getDisplayName()});
                        source.sendSuccess(() -> component, true);
                        tracker.beginCure();
                     } else {
                        MutableComponent component = Component.translatable("commands.witherstormmod.sickness.cureNotInfected");
                        source.sendFailure(component);
                     }
                  } else {
                     MutableComponent component = Component.translatable("commands.witherstormmod.sickness.alreadyBeingCured", new Object[]{living.getDisplayName()});
                     source.sendFailure(component);
                  }
               }
            } else {
