...
public class WitherSicknessTracker extends EntityCapability<WitherSicknessTracker, LivingEntity> {
   ...
   public void tick() {
      if (!this.entity.level().isClientSide) {
         ...
         if (this.shouldUpdate) {
            ...
            WitherStormModPacketHandlers.MAIN.send(SimpleChannel.toTrackingAndSelf(this.entity), message);
      ...
      if (!this.isActuallyImmune()) {
         ...
         if (this.isInfected()) {
            ...
            if (this.delayTicks >= this.getApplicationDelay() && !this.entity.level().isClientSide) {
               ...
               MobEffectInstance effect = this.entity.getEffect(WitherStormModEffects.WITHER_SICKNESS.get());
               if (effect != null && effect.getDuration() < 7200) {
                  MobEffectInstance newEffect = new MobEffectInstance(
                     WitherStormModEffects.WITHER_SICKNESS.get(), 12000, effect.getAmplifier(), false, false, true
                  );
            ...
            if (!this.entity.level().isClientSide
               && !this.entity.hasEffect(WitherStormModEffects.WITHER_SICKNESS.get())
   ...
   public void infect() {
      if (!this.isBeingCured() && !this.isActuallyImmune() && !this.entity.isSpectator()) {
         ...
         MobEffectInstance effect = new MobEffectInstance(
            WitherStormModEffects.WITHER_SICKNESS.get(), 12000, this.getAmplifier(), false, false, true
         );
   ...
   public void cure() {
      if (this.isInfected() && !this.isActuallyImmune()) {
         this.shouldUpdate = true;
         this.entity.removeEffect(WitherStormModEffects.WITHER_SICKNESS.get());
         this.setInfected(false);
         this.setProximityTicks(0);
         this.setCureTicks(0);
         this.countCure();
