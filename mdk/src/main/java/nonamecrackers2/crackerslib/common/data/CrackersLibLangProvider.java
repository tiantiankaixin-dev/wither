...
public class CrackersLibLangProvider extends LanguageProvider {
   public CrackersLibLangProvider(PackOutput output) {
      super(output, "crackerslib", "en_us");
   }

   protected void addTranslations() {
      ConfigLangGeneratorHelper.langForSpec("crackerslib", CrackersLibConfig.CLIENT_SPEC, this, ConfigLangGeneratorHelper.Info.ONLY_RANGE);
      ...
      this.add(
         "config.crackerslib.preset.custom.description",
         "Custom values defined by the user. Changing any value defaulted by a preset will change it to custom."
      );
      ...
      this.add(
         "gui.crackerslib.screen.clientOptions.info",
         "Includes various config options for the client with varying purposes, such as performance and personal preference."
      );
